import { show_popups } from "/js/show_pop_up.js";
import getData from "/js/getBus_data.js";
import { upperDeck, lowerDeck } from "/js/getSeats.js";
import checking_point from "/js/checking_points_popUp.js";
import { patchSeat_upper, patchSeat_lower } from "/js/patch_seatDetail.js";
...
function display_buses(data) {
  ...
  data.forEach((bus) => {
    ...
    <div class="bus_info">
        <table>
            <tr id="tr1">
                <td class="bold"><img class="operator_logo" src="/images/primo_logo.svg" alt=""></td>
            </tr>
            <tr id="tr2">
  ...
  selecter.addEventListener("change", (e) => {
    if (e.target.checked) {
      ...
      display_buses(data); // Reset to original data
    }
  });
...
function DisplayData(data) {
  let newData1 = data.filter(function (time) {
    return time.time_in < "6" && time.time_in >= "12";
  });
...
function seater(data) {
  let newData2 = data.filter(function (name) {
    return name.bus_name.includes("Seater");
  });
...
function sleeper(data) {
  let newData2 = data.filter(function (name) {
    return name.bus_name.includes("Sleeper");
  });
...
function AC(data) {
  let newData2 = data.filter(function (name) {
    return name.bus_name.includes("A/C");
  });
  display_buses(newData2);
}
